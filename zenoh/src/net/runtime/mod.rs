    ...
    pub async fn build(self) -> ZResult<Runtime> {
        ...
        let transport_manager_builder = TransportManager::builder()
            ...
            .bound_callback({
                ...
                move |p| region::compute_transient_region_of(&p, &config).map(|r| r.bound())
            });
    ...
    fn new_unicast(
        ...
        match zread!(self.runtime).upgrade().as_ref() {
            Some(runtime) => {
                ...
                let transient_region = region::compute_transient_region_of(&peer, &config)?;

                let region = region::compute_region_of(
                    &peer,
                    &config,
                    &transient_region,
                    &transport.get_bound()?,
